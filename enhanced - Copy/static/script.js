...
function activateSiren() {
    // Play siren sound - try HTML audio first, then Web Audio API fallback
    const sirenSound = document.getElementById('sirenSound');
    if (sirenSound) {
        sirenSound.volume = 1.0;
        sirenSound.play().catch(e => {
            console.log('HTML audio not available, using Web Audio API');
            playSirenWebAudio();
        });
    } else {
        playSirenWebAudio();
...
function stopSiren() {
    ...
    if (sirenGain && sirenGain.sirenInterval) {
